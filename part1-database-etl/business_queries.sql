...
-- total quantity sold, and total revenue generated. Only include categories that have generated 
...
-- Business Question: "Show monthly sales trends for the year 2024. For each month, display 
-- the month name, total number of orders, total revenue, and the running total of revenue 
...
SELECT
    ...
    SUM(SUM(oi.quantity * oi.unit_price)) OVER (ORDER BY MONTH(o.order_date)) AS cumulative_revenue
FROM orders o
JOIN order_items oi ON o.order_id = oi.order_id
